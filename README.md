...
```bash
trunk build --release
wasm-opt -O4 -all --disable-compact-imports dist/katvisualizer_wasm_bg.wasm -o dist/katvisualizer_wasm_bg.opt.wasm
...
- Render Options -> Spectrogram duration (ideal value depends on what you're trying to analyze; increasing it may hurt performance)
...
- Render Options -> Bargraph averaging (if you have a >=100Hz monitor, set this to 10ms)
...
(Note: If you're having trouble parsing complex sounds using the spectrogram, treat the spectrogram like a top-down view of a moving 3D object where lightness indicates depth. Pay attention to how changes in the shape of the 3D object change the sound you hear, ideally while listening using an audio device with very low latency.)
